...
var NinjaPrinterCore = {
  ...
  _initBackgroundListener: function () {
    chrome.runtime.onMessage.addListener(function (
      ...
      if (request.action === "print") {
        customEvent = new CustomEvent("ninjaprinter.result", {
    ...
    window.addEventListener(
      ...
      function (event) {
        ...
        if (!NinjaPrinterCore._validateRequest(event.data)) {
          //   return console.info(
          //     "Invalid request, make sure all parameters are set."
          //   );
          return;
        }
  ...
  _validateRequest: function (request) {
    ...
    if (
      request.printerType.toUpperCase() != "LABEL" &&
      request.printerType.toUpperCase() != "PDF"
    ) {
      return false;
    }
